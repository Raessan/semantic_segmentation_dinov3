...
def visualize_maps(image,
    ...
    Args:
        ...
        draw_instance_ids: annotate instances on panoptic panel (uid:name)
        draw_semantic_labels: annotate semantic panel with category names
        semantic_label_fontsize: fontsize for semantic labels
    Returns:
    ...
    sem_list = []  # store (cls, centroid_y, centroid_x, area)
    for cls in unique_c:
        ...
        # compute centroid for annotation (use mean of coordinates)
        ys, xs = np.nonzero(mask)
        if ys.size > 0:
            y0 = int(np.mean(ys))
            x0 = int(np.mean(xs))
            sem_list.append((int(cls), y0, x0, int(mask.sum())))

    axes[1].imshow(out_sem)
    axes[1].set_title("Semantic map")
    axes[1].axis('off')

    # annotate semantic class names (one label per class, placed at centroid of mask)
    if draw_semantic_labels:
        for cls, y0, x0, area in sem_list:
            label = class_names[cls] if (class_names is not None and cls < len(class_names)) else str(cls)
            # white text with black translucent bbox for readability
            axes[1].text(x0, y0, f"{label}", color='white', fontsize=semantic_label_fontsize,
                         ha='center', va='center',
